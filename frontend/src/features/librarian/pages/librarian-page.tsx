export function LibrarianPage() {
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      <h1 className="text-2xl font-bold text-slate-900">Librarian Workspace</h1>
      <p className="mt-2 text-slate-600">Circulation desk, inventory operations, and task queues.</p>
      <div className="mt-6 rounded-lg border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-500">
        Feature scaffold is ready. Business logic and UI flows will be implemented in iterative phases.
      </div>
    </section>
  );
}