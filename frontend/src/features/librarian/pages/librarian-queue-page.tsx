...
import { useLibrarianReservationQueue, useUpdateReservationStatus } from "@features/reservations/hooks/use-reservations";
...
const STATUS_COLORS: Record<ReservationStatus, { badge: string; label: TranslationKey }> = {
  PENDING: { badge: "bg-yellow-50 text-yellow-800 border-yellow-100", label: "librarianQueueStatusPending" },
  READY: { badge: "bg-blue-50 text-blue-800 border-blue-100", label: "librarianQueueStatusReady" },
  FULFILLED: { badge: "bg-green-50 text-green-800 border-green-100", label: "librarianQueueStatusFulfilled" },
  CANCELLED: { badge: "bg-gray-50 text-gray-800 border-gray-100", label: "librarianQueueStatusCancelled" },
  EXPIRED: { badge: "bg-red-50 text-red-800 border-red-100", label: "librarianQueueStatusCancelled" },
};
...
export function LibrarianQueuePage() {
  ...
  const { data, isLoading, error } = useLibrarianReservationQueue(statusFilter, page, 20);
  ...
  if (!authStore.isAuthenticated || (authStore.role !== "LIBRARIAN" && authStore.role !== "ADMIN")) {
    return (
    ...
    <section className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        >
          ...
          <option value="FULFILLED">{t("librarianQueueStatusFulfilled")}</option>
          <option value="CANCELLED">{t("librarianQueueStatusCancelled")}</option>
        </select>
      </div>
              ...
              return (
                <tr key={reservation.id} className="border-b border-slate-100 hover:bg-slate-50">
                  <td className="px-4 py-3 text-slate-900">
                    ...
                    <div className="text-xs text-slate-600">{reservation.userId}</div>
                  </td>
                  <td className="px-4 py-3 text-slate-900">
                  ...
                  <td className="px-4 py-3">
                    <div className="space-x-2 flex">
                      {reservation.status === "PENDING" && (
                        <>
                          <button
                            onClick={() => handleStatusChange(reservation.id, "READY")}
                      ...
                      {!["PENDING", "READY"].includes(reservation.status) && (
                        <span className="text-xs text-slate-400">{t("librarianQueueNoActions")}</span>
                      )}
                    </div>
                  </td>
                </tr>
