...
| Public | Events Calendar | TBD (future canonical export) | `/events` | `events/index.blade.php` (planned) | planned | Distinct module from news; calendar/list surface for upcoming and past library events. |
| Public | Event Detail | TBD (future canonical export) | `/events/{slug}` | `events/show.blade.php` (planned) | planned | Event-specific detail page; not merged into news detail. |
| Public | Library Leadership | TBD (future canonical export) | `/leadership` | `leadership.blade.php` (planned) | planned | Standalone public informational page for library leadership/administration in polished editorial card format. |
| Public | Library Usage Rules | TBD (future canonical export) | `/rules` | `rules.blade.php` (planned) | planned | Standalone public informational page for reader policy and usage rules. |
| Public | Location & Wayfinding | About/Contacts + map component (future canonical export TBD) | `/contacts` (enhanced) | `about.blade.php` or dedicated location view (planned) | planned | Must explicitly cover how to find the library and room-level fund locations: `1/200` (технологический фонд), `1/202` (фонд колледжа), `1/203` (экономический фонд библиотеки), plus map component. |
| Public | Discover | Project A / Academic Discovery Hub | `/discover` | `discover.blade.php` | pending | Keep UDC-first logic. Scope refinement: add knowledge-map entry (faculty/department current directions) as secondary discovery axis under UDC. |
