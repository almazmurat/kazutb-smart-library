...
|---|---|---|---|---|---|---|
| Public | Homepage | `docs/design-exports/homepage_canonical/` | `/` | `welcome.blade.php` | **implemented — Phase 3.h canonical-exact rebuild** | Phase 3.h — canonical-exact rebuild per `homepage_canonical`. Old 8-section Enhanced Homepage shell (operational hours, news/announcements, Pulse of Innovation stats, institutional repository, research guides, latest arrivals, contact card) replaced wholesale. New 3-section layout: (1) Hero — 7/12 col with kicker + Newsreader h1 + Manrope lead + hero search form (`id="heroSearch"` + `class="hero-search-bar"`) + topic quick-links (`id="hero-quick-links"`) + `#hero-campus-mark` identity div; 5/12 col with campus image + floating stats card (backdrop-blur). (2) Bento Collections — 3-col grid, large tile (md:col-span-2) → `/discover`, two small tiles → `/catalog?udc=5` and `/catalog?udc=33`. (3) Scholarly Services — 3 service cards with Material Symbols icons; guest service 3 links to `/catalog`, auth service 3 links to `/dashboard`. Tri-lingual ru/kk/en `$chrome` data keyed per locale. Backward-compat markers preserved for ConsolidationTest: `id="heroSearch"`, `class="hero-search-bar"`, `id="hero-quick-links"`, `#hero-campus-mark` div with `.campus-mark__logo` and "Библиотека КазТБУ". |
